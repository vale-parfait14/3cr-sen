...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://localhost:5002/users'); // API avec /users
  ...
  const handleSubmit = async (e) => {
    ...
    if (editUserId !== null) {
      ...
      try {
        await axios.put(`http://localhost:5002/users/${users[editUserId]._id}`, userData); // API PUT
      ...
      try {
        await axios.post('http://localhost:5002/users', userData); // API POST
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5002/users/${id}`); // API DELETE
  ...
  return (
    <div className="container mt-4 ">
      ...
      <button onClick={() => navigate('/patients')} className="btn btn-secondary w-100 mt-3">Retour</button>
