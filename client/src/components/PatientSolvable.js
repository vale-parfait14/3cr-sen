...
const PatientSolvable = ({ patients }) => {
  ...
  useEffect(() => {
    const fetchPayments = async () => {
      try {
        const response = await fetch('https://threecr-sen-1.onrender.com/api/payments');
        if (response.ok) {
...
useEffect(() => {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`https://threecr-sen-1.onrender.com/api/payments/${editing}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async (paymentId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce paiement ?')) {
      try {
        const response = await fetch(`https://threecr-sen-1.onrender.com/api/payments/${paymentId}`, {
          method: 'DELETE'
        });
